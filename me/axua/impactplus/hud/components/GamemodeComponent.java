...
public class GamemodeComponent extends Panel {
  Gamemode mod;
  ...
  public GamemodeComponent(double ix, double iy, ClickGUI parent) {
    super("Gamemode", ix, iy, 10.0D, 10.0D, false, parent);
    this.mod = (Gamemode)ModuleManager.getModuleByName("Gamemode");
  ...
  public void drawHud() {
    ...
    String s = mc.playerController.getCurrentGameType().getName();
    String biome11 = s.substring(0, 1).toUpperCase() + s.substring(1);
    String biom = "Gamemode " + numbercf + biome11;
    float hi = ImpactPlus.CustomFont2.getStringWidth(biom);
    float hi2 = mc.fontRenderer.getStringWidth(biom);
    if (this.mod.right.getValBoolean()) {
      if (this.font) {
        ImpactPlus.CustomFont2.drawStringWithShadow(biom, (float)this.x - hi, (float)this.y, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(biom, (float)this.x - hi2, (float)this.y, this.text.getRGB());
      } 
    } else if (this.font) {
      ImpactPlus.CustomFont2.drawStringWithShadow(biom, (float)this.x, (float)this.y, this.text.getRGB());
    } else {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    String biome11 = s.substring(0, 1).toUpperCase() + s.substring(1);
    String biom = "Gamemode " + numbercf + biome11;
    double w = (ImpactPlus.CustomFont2.getStringWidth(biom) + 2.0F);
    ...
    if (this.extended) {
      ...
      if (this.mod.right.getValBoolean()) {
        if (this.font) {
          ImpactPlus.CustomFont2.drawStringWithShadow(biom, (float)this.x - hi, (float)this.y + 10.0F, this.text.getRGB());
        } else {
          mc.fontRenderer.drawStringWithShadow(biom, (float)this.x - hi2, (float)this.y + 10.0F, this.text.getRGB());
        } 
      } else if (this.font) {
        ImpactPlus.CustomFont2.drawStringWithShadow(biom, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
      } else {
