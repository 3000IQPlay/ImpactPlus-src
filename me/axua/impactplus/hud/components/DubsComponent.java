...
public class DubsComponent extends Panel {
  DubCount mod;
  ...
  public DubsComponent(double ix, double iy, ClickGUI parent) {
    super("DubCount", ix, iy, 10.0D, 10.0D, false, parent);
    this.mod = (DubCount)ModuleManager.getModuleByName("DubCount");
  ...
  public void drawHud() {
    ...
    long chest = mc.world.loadedTileEntityList.stream().filter(e -> e instanceof net.minecraft.tileentity.TileEntityChest).count();
    int dub = (int)(chest / 2L);
    String dub2 = "" + dub;
    String chest2 = "" + chest;
    if (this.mod.mode.getValString().equals("Dubs")) {
      this.serve = "DubCount " + numbercf + dub2;
    } else {
      this.serve = "ChestCount " + numbercf + chest2;
    } 
    float hi = ImpactPlus.CustomFont2.getStringWidth(this.serve);
    float hi2 = mc.fontRenderer.getStringWidth(this.serve);
    if (this.mod.right.getValBoolean()) {
      if (this.font) {
        ImpactPlus.CustomFont2.drawStringWithShadow(this.serve, (float)this.x - hi, (float)this.y, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(this.serve, (float)this.x - hi2, (float)this.y, this.text.getRGB());
      } 
    } else if (this.font) {
      ImpactPlus.CustomFont2.drawStringWithShadow(this.serve, (float)this.x, (float)this.y, this.text.getRGB());
    } else {
      mc.fontRenderer.drawStringWithShadow(this.serve, (float)this.x, (float)this.y, this.text.getRGB());
    } 
  }
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    double w = (ImpactPlus.CustomFont2.getStringWidth(this.serve) + 2.0F);
    ...
    if (this.extended) {
      ...
      if (this.mod.right.getValBoolean()) {
        if (this.font) {
          ImpactPlus.CustomFont2.drawStringWithShadow(this.serve, (float)this.x - hi, (float)this.y + 10.0F, this.text.getRGB());
        } else {
          mc.fontRenderer.drawStringWithShadow(this.serve, (float)this.x - hi2, (float)this.y + 10.0F, this.text.getRGB());
        } 
      } else if (this.font) {
        ImpactPlus.CustomFont2.drawStringWithShadow(this.serve, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(this.serve, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
      } 
    } 
  }
