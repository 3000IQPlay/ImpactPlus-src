...
public class TotemsComponent extends Panel {
  ...
  public TotemsComponent(double ix, double iy, ClickGUI parent) {
    super("Totems", ix, iy, 10.0D, 10.0D, false, parent);
    this.mod = (Totems)ModuleManager.getModuleByName("Totems");
  ...
  public void drawHud() {
    ...
    if (this.mod.mode.getValString().equalsIgnoreCase("Item")) {
      ItemStack is;
      if (this.totems == 0) {
        is = new ItemStack(Items.TOTEM_OF_UNDYING);
      } else {
        is = new ItemStack(Items.TOTEM_OF_UNDYING, this.totems);
      } 
      ...
      String totemss = "" + this.totems;
      String totemsss = numbercf + totemss;
      FontUtils.drawStringWithShadow(this.font, totemsss + "", (int)this.x + 16, (int)this.y + 9 - FontUtils.getFontHeight(this.font) / 2, this.text.getRGB());
    } else {
      float hi = ImpactPlus.CustomFont2.getStringWidth(this.tot);
      float hi2 = mc.fontRenderer.getStringWidth(this.tot);
      if (this.mod.right.getValBoolean()) {
        if (this.font) {
          ImpactPlus.CustomFont2.drawStringWithShadow(this.tot, (float)this.x - hi, (float)this.y, this.text.getRGB());
        } else {
          mc.fontRenderer.drawStringWithShadow(this.tot, (float)this.x - hi2, (float)this.y, this.text.getRGB());
        } 
      } else if (this.font) {
        ImpactPlus.CustomFont2.drawStringWithShadow(this.tot, (float)this.x, (float)this.y, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(this.tot, (float)this.x, (float)this.y, this.text.getRGB());
      } 
    } 
  }
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    doStuff();
    double w = (ImpactPlus.CustomFont2.getStringWidth(this.tot) + 2.0F);
    ...
    if (this.extended) {
      ...
      if (this.mod.mode.getValString().equalsIgnoreCase("Item")) {
        ...
        String totemss = "" + this.totems;
        String totemsss = numbercf + totemss;
        FontUtils.drawStringWithShadow(this.font, totemsss + "", (int)this.x + 16, (int)this.y + 19 - FontUtils.getFontHeight(this.font) / 2, this.text.getRGB());
      } else {
        float hi = ImpactPlus.CustomFont2.getStringWidth(this.tot);
        float hi2 = mc.fontRenderer.getStringWidth(this.tot);
        if (this.mod.right.getValBoolean()) {
          if (this.font) {
            ImpactPlus.CustomFont2.drawStringWithShadow(this.tot, (float)this.x - hi, (float)this.y + 10.0F, this.text.getRGB());
          } else {
            mc.fontRenderer.drawStringWithShadow(this.tot, (float)this.x - hi2, (float)this.y + 10.0F, this.text.getRGB());
          } 
        } else if (this.font) {
          ImpactPlus.CustomFont2.drawStringWithShadow(this.tot, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
        } else {
          mc.fontRenderer.drawStringWithShadow(this.tot, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
        } 
      } 
    } 
  }
  ...
  private void doStuff() {
    ...
    if (this.mod.mode.getValString().equalsIgnoreCase("Short")) {
      this.tot = "TOT " + numbercf + this.totems;
    } else {
      this.tot = "Totems " + numbercf + this.totems;
    } 
  }
}
