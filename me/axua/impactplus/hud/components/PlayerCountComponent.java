...
public class PlayerCountComponent extends Panel {
  PlayerCount mod;
  ...
  public PlayerCountComponent(double ix, double iy, ClickGUI parent) {
    super("PlayerCount", ix, iy, 10.0D, 10.0D, false, parent);
    this.mod = (PlayerCount)ModuleManager.getModuleByName("PlayerCount");
  ...
  public void drawHud() {
    ...
    String playerCount = "Online " + numbercf + (Minecraft.getMinecraft()).player.connection.getPlayerInfoMap().size();
    doStuff();
    String okcool = playerCount;
    float hi = ImpactPlus.CustomFont2.getStringWidth(okcool);
    float hi2 = mc.fontRenderer.getStringWidth(okcool);
    if (this.mod.right.getValBoolean()) {
      if (this.font) {
        ImpactPlus.CustomFont2.drawStringWithShadow(okcool, (float)this.x - hi, (float)this.y, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(okcool, (float)this.x - hi2, (float)this.y, this.text.getRGB());
      } 
    } else if (this.font) {
      ImpactPlus.CustomFont2.drawStringWithShadow(okcool, (float)this.x, (float)this.y, this.text.getRGB());
    } else {
      mc.fontRenderer.drawStringWithShadow(okcool, (float)this.x, (float)this.y, this.text.getRGB());
    } 
  }
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    doStuff();
    String okcool = playerCount;
    double w = (ImpactPlus.CustomFont2.getStringWidth(okcool) + 2.0F);
    ...
    if (this.extended) {
      ...
      if (this.mod.right.getValBoolean()) {
        if (this.font) {
          ImpactPlus.CustomFont2.drawStringWithShadow(okcool, (float)this.x - hi, (float)this.y + 10.0F, this.text.getRGB());
        } else {
          mc.fontRenderer.drawStringWithShadow(okcool, (float)this.x - hi2, (float)this.y + 10.0F, this.text.getRGB());
        } 
      } else if (this.font) {
        ImpactPlus.CustomFont2.drawStringWithShadow(okcool, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
      } else {
        mc.fontRenderer.drawStringWithShadow(okcool, (float)this.x, (float)this.y + 10.0F, this.text.getRGB());
      } 
    } 
  }
