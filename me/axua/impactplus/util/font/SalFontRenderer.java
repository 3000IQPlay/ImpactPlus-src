...
public class SalFontRenderer {
  ...
  public SalFontRenderer(String fontName, float fontSize) {
    ...
    try {
      ...
      this.unicodeFont = new UnicodeFont(getFontByName(fontName).deriveFont(fontSize * this.prevScaleFactor / 2.0F));
      this.unicodeFont.addAsciiGlyphs();
      this.unicodeFont.getEffects().add(new ColorEffect(Color.WHITE));
      this.unicodeFont.loadGlyphs();
    } catch (FontFormatException|IOException|org.newdawn.slick.SlickException e) {
      e.printStackTrace();
    } 
  ...
  public static Font getFontByName(String name) throws IOException, FontFormatException {
    return getFontFromInput("/assets/osiris/fonts/Muli-SemiBold.ttf");
  }
  
  public static Font getFontFromInput(String path) throws IOException, FontFormatException {
    InputStream test = SalFontRenderer.class.getResourceAsStream(path);
    System.out.println(path);
    System.out.println(test.toString());
    return Font.createFont(0, test);
  }
  ...
  public float getWidth(String text) {
    ...
    return ((Float)this.cachedStringWidth.computeIfAbsent(text, e -> Float.valueOf(this.unicodeFont.getWidth(ChatColor.stripColor(text)) / this.antiAliasingFactor))).floatValue();
  }
  ...
  public float getStringWidth(String p_Name) {
    return (this.unicodeFont.getWidth(ChatColor.stripColor(p_Name)) / 2);
  }
